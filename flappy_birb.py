...
class Bird:
    ...
    def draw(self, win):
        ...
        # bird isn't flapping when diving
        if self.tilt <= -80:
            self.img = self.IMGS[1]
...
def draw_win(win, birds, pipes, base, score, gen, pipe_ind):
    ...
    win.blit(score_txt, (WIN_WIDTH - score_txt.get_width() - 15, 10))
    # gens
    gen_txt = STAT_FONT.render('Gen: ' + str(gen-1), 1, (255, 255, 255))
    win.blit(gen_txt, (10, 10))
    # birds alive
    alive_txt = STAT_FONT.render(
        'Alive: ' + str(len(birds)), 1, (255, 255, 255))
    win.blit(alive_txt, (10, 50))

    pygame.display.update()


def birb(genomes, config):
